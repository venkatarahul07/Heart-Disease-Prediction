...
df = pd.read_csv(Path(__file__).with_name('heartdisease.csv'))
# DROPS DUPLICATE
...
if (selected == 'Heart Disease Prediction'):
    ...
    with col1:
        ...
        thal = st.number_input("Thalassemia type (0-3)", 0, 3, step=1)
    with col2:
    ...
    with col3:
        Chest_Pain = st.number_input("Chest Pain Type (0-3)", 0, 3, step=1)
        fasting_sugar = st.number_input("Fasting blood sugar > 120 mg/dL (0=No, 1=Yes)", 0, 1, step=1)
        exang = st.number_input("exercise induced angina (0,1)", 0, 1)
        ca = st.number_input("number of major vessels (0-4) colored by fluoroscopy", 0, 4, step=1)
...
if (selected == 'Contact Me'):
    ...
    if st.button("Submit"):
        if name.strip() == "" or email.strip() == "" or message.strip() == "":
            ...
            send_email_to = 'h.dhanush2007@gmail.com'
